...
\begin{itemize}
\item \textbf{Bootstrap aggregating (bagging):} Each tree is trained on a random sample (with replacement) from the 141-sample training set. This introduces diversity; trees trained on different subsamples discover different patterns.
...
Despite achieving $R^2 = 0.9245$ on the fixed seed-42 split (competitive with RF), MLP demonstrates instability across the 30-fold validation: mean RMSE = 6.22 mM with std = 1.54 mM, compared to RF's mean 5.51 mM with std = 1.08 mM. This 40\% higher variance reflects fundamental challenges of neural network training on small tabular data.
...
The MLP architecture has approximately 2,432 parameters (weights + biases): $7 \times 64 + 64 \times 32 + 32 \times 1 = 2,432$ after accounting for bias terms. The training set has only 141 samples. The ratio of parameters to training samples is $2432 / 141 \approx 17$, meaning the model has 17 times as many parameters as training samples. This is unfavorable; a rule of thumb (though not absolute) is that parameters should not exceed training samples.
...
Our test set has 35 samples (20\% of 176). For a metric such as RMSE, the confidence interval width is approximately:
\[
\text{CI width} \propto \frac{\sigma}{\sqrt{n_{\text{test}}}} \approx \frac{4.37}{\sqrt{35}} \approx 0.74 \text{ mM}
\]
...
The 30-fold repeated random-split validation provides robust evidence that RF generalizes well. Across 30 different 80-20 partitions of the data, RF consistently achieves RMSE in the range 3.81–8.24 mM (mean 5.51 mM). This narrow range suggests the model is not overfitting to a particular split; rather, it captures patterns stable across data resampling. This is strong validation of generalization.

Ideally, k-fold cross-validation (k=5 or k=10, where data are divided into k folds and the model is trained k times, leaving each fold out once) would provide additional robustness. However, the repeated random-split approach (30 folds) is equally rigorous and offers more flexibility in split proportions (we used 80-20; k-fold would typically use 90-10).
...
\begin{enumerate}
\item \textbf{Larger training dataset:} Expanding from 141 to 300+ training samples would stabilize MLP, enable better hyperparameter tuning (via grid search on separate validation set), and may improve RF further.
