...
\begin{itemize}
\item Time (experimental duration): cumulative fermentation time in the released dataset, range 0.00--8.57
\item Gas composition: N2, CO, CO2, H2 (all in mole fractions or mol\%): represent syngas fed to the bioreactor
\item Flow rate (mL/min): gas volumetric flow through the culture, range 1.00--20.00 mL/min
...
\item Range: 0.00--108.94 mM (mean $\approx 22.25$ mM, std $\approx 20.73$ mM)
\item Measured offline via high-performance liquid chromatography (HPLC) or gas chromatography
\end{itemize}

\subsection{Descriptive Statistics of the Supervised Task}
Table~\ref{tab:descstats} summarizes mean, standard deviation, and range for all supervised inputs and target labels used in training/testing.

\begin{table}[htbp]
\caption{Descriptive Statistics of Input Features and Target}
\label{tab:descstats}
\centering
\begin{tabular}{lrrrr}
\toprule
...
\midrule
time & 2.3188 & 2.0251 & 0.00 & 8.57 \\
N2 & 23.3807 & 22.9488 & 0.00 & 65.00 \\
CO & 34.7727 & 13.6886 & 20.00 & 50.00 \\
CO2 & 25.6818 & 10.9852 & 15.00 & 37.50 \\
H2 & 16.1648 & 8.3619 & 0.00 & 25.00 \\
flow rate (mL/min) & 14.6534 & 7.5244 & 1.00 & 20.00 \\
composition & 5.0795 & 2.8932 & 1.00 & 10.00 \\
ethanol (mM) & 22.2484 & 20.7287 & 0.00 & 108.94 \\
\bottomrule
\end{tabular}
\end{table}

These statistics indicate heterogeneous feature scales (for example, N2 standard deviation is much larger than composition), motivating explicit scaling analysis for gradient-based models.
...
\subsection{Data Sufficiency and Active Learning Consideration}
With 176 total observations and 7 predictors, this project is in the small-data regime for high-capacity neural models. We partially mitigate this with repeated random holdout evaluation and paired statistical testing instead of relying on a single split. If additional labels are costly, active learning is a practical extension: query the next experiment near high-uncertainty regions (for example, disagreement between RF and MLP predictions) to maximize information gained per wet-lab run.
...
\subsection{Software Stack and Library Choice}
All implementations use open-source Python libraries: scikit-learn (modeling and preprocessing), scipy (hypothesis tests), pandas/numpy (data operations), and matplotlib (visualization). We selected scikit-learn for reliable, reproducible baseline implementations across LR, RF, and MLP under a consistent API.
...
\subsection{Alternatives Surveyed and Design Choices}
We considered several alternatives but prioritized reproducible core comparisons for this submission:
\begin{itemize}
\item \textbf{Feature-set alternatives:} ratio features (e.g., CO/H2), interaction terms (CO$\times$H2), and lag/temporal transforms were considered but deferred to avoid overfitting under limited sample size.
\item \textbf{Model alternatives:} gradient boosting, XGBoost-like tree ensembles, SVR, and deeper MLPs were considered as potential extensions.
\item \textbf{Training alternatives:} full hyperparameter grid search and nested CV were considered; we instead used fixed core hyperparameters plus repeated holdout for stable comparison within available time.
\end{itemize}
...
Random Forest makes splits on feature thresholds, independent of feature magnitude. This contrasts with distance-based models (MLP, k-NN) and linear models, which are sensitive to feature scaling. In our dataset, feature ranges are heterogeneous (e.g., N2 spans 0--65 while composition spans 1--10). MLP requires StandardScaler to normalize these features; RF requires none. This reduces hyperparameter tuning burden and avoids data leakage from scaling a validation set by training statistics.
...
\item \textbf{Pre-trained vision/CV route for multimodal expansion:} If future systems include reactor image streams (foam, turbidity, bubble patterns), evaluate pre-trained or fine-tuned computer-vision backbones and fuse visual embeddings with process signals for inference on free-tier commercial platforms.
\end{enumerate}
